...
GLB-Dateien (glTF-Binary) mit gebackener Bild-Textur um. Laeuft als Blender-Skript im
...
Ablauf pro Datei:
  ...
  5. GLB mit eingebetteter Textur exportieren
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="PLY -> GLB mit gebackener Textur")
    parser.add_argument("--input", required=True,
                         help="Pfad zu einer .ply-Datei ODER einem Ordner voller .ply-Dateien")
    parser.add_argument("--output", required=True,
                         help="Zielordner fuer die fertige .glb")
    ...
    parser.add_argument("--rename", default=None,
                         help="Ausgabedateiname (ohne Endung) ueberschreiben, statt den "
                              "urspruenglichen Dateinamen zu behalten")
...
def process_file(filepath, output_dir, bake_size, margin, rename=None):
    name = rename or os.path.splitext(os.path.basename(filepath))[0]
...
def main():
    ...
    for filepath in ply_files:
        process_file(filepath, args.output, args.bake_size, args.margin, args.rename)
