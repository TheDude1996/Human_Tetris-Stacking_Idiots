...
Unity-Asset (GLB). Deckt ab:
  ...
  5. Export:        GLB (glTF-Binary) mit eingebetteter Textur, fertig fuer Unity-Import
...
Nutzung (Kommandozeile):
  ...
  # (z.B. S.ply -> Z.glb), ohne sie erneut zu scannen:
  blender --background --python scan_to_unity_pipeline.py -- \
...
Wichtige Flags:
  ...
  --output          Zielordner fuer die fertige .glb (Textur ist eingebettet)
...
Kompatibilitaet: Getestet fuer Blender 5.2 LTS. Die Vertex-Farben werden ueber
die color_attributes-API ausgelesen (die alte vertex_colors-API wurde in
Blender 4.0 entfernt/ist seitdem immer leer).
"""
...
def parse_args():
    ...
    p = argparse.ArgumentParser(description="Scan (PLY/STL) -> bereinigtes, texturiertes Unity-GLB")
...
def get_color_attribute_name(obj):
    """Ermittelt den Namen der Vertex-Color-Schicht, kompatibel mit der
    color_attributes-API (Blender 4.0+/5.x). Die alte vertex_colors-API
    existiert in Blender 5.2 LTS nicht mehr bzw. ist immer leer, auch wenn
    das Mesh Farben hat -- deshalb zuerst color_attributes pruefen."""
    color_attrs = getattr(obj.data, "color_attributes", None)
    if color_attrs and len(color_attrs) > 0:
        return color_attrs[0].name
    # Fallback fuer sehr alte Blender-Versionen (< 3.2)
...
def build_vertex_color_material(obj, color_attr_name):
    ...
    # ShaderNodeVertexColor.layer_name greift in Blender 4.x/5.x auf die
    # color_attributes-Schicht des Meshes zu (Name muss exakt uebereinstimmen).
