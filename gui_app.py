...
class PipelineGUI(tk.Tk):
    def __init__(self):
        ...
        self.rename_var = tk.StringVar(value=self.config_data.get("rename", ""))
    ...
    def _build_run_tab(self, parent):
        ...
        ttk.Label(opt_frame, text="Umbenennen (optional):").grid(row=1, column=0, sticky="w", padx=8, pady=4)
        ttk.Entry(opt_frame, textvariable=self.rename_var, width=12).grid(row=1, column=1, sticky="w", pady=4)
    ...
    def _build_command(self):
        ...
        if script_name == "scan_to_unity_pipeline.py":
            ...
            if self.rename_var.get():
                cmd += ["--rename", self.rename_var.get()]
    ...
    def _run_process(self, cmd):
        ...
        self.after(0, self._on_process_finished, return_code)
