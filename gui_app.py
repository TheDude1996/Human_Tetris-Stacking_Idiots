...
BUILTIN_SCRIPTS = {
    ...
    "ply_to_cube_textured.py (Textur auf Wuerfel backen)": "ply_to_cube_textured.py",
}
...
class PipelineGUI(tk.Tk):
    def __init__(self):
        ...
        self.edge_length_var = tk.DoubleVar(value=self.config_data.get("edge_length", 1.0))
    ...
    def _build_run_tab(self, parent):
        ...
        ttk.Label(opt_frame, text="Modellgroesse / Wuerfel-Kantenlaenge (m):").grid(
            row=1, column=0, columnspan=2, sticky="w", padx=8, pady=(10, 0))
        self.edge_length_value_label = ttk.Label(opt_frame, text=f"{self.edge_length_var.get():.2f} m")
        self.edge_length_value_label.grid(row=1, column=2, sticky="e", padx=8, pady=(10, 0))

        edge_length_slider = ttk.Scale(
            opt_frame, from_=0.1, to=6.0, orient="horizontal",
            variable=self.edge_length_var, command=self._on_edge_length_changed,
        )
        edge_length_slider.grid(row=2, column=0, columnspan=3, sticky="we", padx=8, pady=(0, 6))
        ...
        ttk.Label(
            opt_frame,
            text="Wirkt nur bei ply_to_cube_textured.py: legt die feste Wuerfel-Kantenlaenge fest "
                 "und verkleinert das Modell nur bei Bedarf, damit es hineinpasst.",
    ...
    def _on_edge_length_changed(self, _value=None):
        self.edge_length_value_label.configure(text=f"{self.edge_length_var.get():.2f} m")
    ...
    def _build_command(self, input_path, output_dir, rename_name):
        ...
        if script_name == "ply_to_cube_textured.py":
            cmd += ["--edge-length", f"{self.edge_length_var.get():.4f}"]
