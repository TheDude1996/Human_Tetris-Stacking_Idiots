...
Ablauf pro Datei:
  ...
  5. Nur den Wuerfel (nicht den Original-Scan) als GLB (glTF-Binary) mit
     eingebetteter Textur exportieren
...
Wichtige Flags:
  ...
  --edge-length 1.0         Feste Wuerfel-Kantenlaenge in Metern (z.B. vom
                            GUI-Schieberegler). Ueberschreibt --cube-mode und
                            die Bounding-Box-basierte Groesse. Das Mesh wird
                            IMMER gleichmaessig so skaliert, dass es genau in
                            diesen Wuerfel passt -- verkleinert bei zu grossen
...
def parse_args():
    ...
    p = argparse.ArgumentParser(description="PLY-Scan -> texturierter Wuerfel (GLB)")
    ...
    p.add_argument("--edge-length", type=float, default=None,
                    help="Feste Kantenlaenge des Wuerfels in Metern (z.B. aus dem GUI-Schieberegler). "
                         "Ueberschreibt --cube-mode/--padding-basierte Groessenberechnung. Das Mesh wird "
                         "IMMER gleichmaessig so skaliert, dass es genau hineinpasst -- verkleinert oder vergroessert.")
...
def create_bake_cube(center, dims, cube_mode, padding, exact_edge_length=None):
    if exact_edge_length is not None:
        size_x = size_y = size_z = exact_edge_length
    else:
...
def scale_source_to_fit(source_obj, dims, edge_length, padding):
    """Skaliert das Mesh gleichmaessig so, dass seine groesste Ausdehnung genau
    in den Ziel-Wuerfel (Kantenlaenge edge_length, abzueglich Sicherheitsabstand
    padding) passt -- verkleinert grosse Meshes UND vergroessert kleine, damit
    am Ende immer dieselbe relative Groesse im Wuerfel erreicht wird."""
    ...
    if abs(scale_factor - 1.0) > 1e-6:
        ...
        source_obj.scale = tuple(c * scale_factor for c in source_obj.scale)
        bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)
...
def process_file(filepath, args):
    ...
    if args.edge_length is not None:
        scale_source_to_fit(source_obj, dims, args.edge_length, args.padding)
        # Nach evtl. Skalierung erneut berechnen, da sich Mittelpunkt/Ausdehnung
        # je nach Objekt-Ursprung leicht verschieben koennen.
        center, dims = compute_world_bounds(source_obj)
        cube_obj, cube_side = create_bake_cube(center, dims, args.cube_mode, args.padding,
                                                exact_edge_length=args.edge_length)
    else:
        cube_obj, cube_side = create_bake_cube(center, dims, args.cube_mode, args.padding)
    ...
    # Unity beim GLB-Import direkt die richtige Albedo-Textur bekommt.
    glb_path = os.path.join(args.output, f"{name}.glb")
    export_glb(cube_obj, glb_path)
    print(f"  Fertig: {glb_path}  (Textur eingebettet, zusaetzlich als PNG: {image_path})")
